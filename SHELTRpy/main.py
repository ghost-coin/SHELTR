...
async def getMax():
    ...
    if not txHistory.wallet.utxo:
        return
    maxAmount = await TransactionInputs(
        txHistory, 0, spendColdStake=spendCSOut
    ).getMax()

    if maxAmount >= MIN_TX:
        Element("send-tab-amount").element.value = txHistory.util.convertFromSat(
            maxAmount
        )
...
async def doZap():
    ...
    Element(
        "message-box"
    ).element.innerHTML = f"""<h3>{locale['processing-transaction']}</h3><p>{locale['pool']}:</p><p class="message-box-secondary"></p><p>{locale['send-tab-amount']}:</p><p class="message-box-secondary"></p>
    ...
    maxAmount = txHistory.util.convertFromSat(
    ...
    Element(
        "message-box"
    ).element.innerHTML = f"""<h3>{locale['please-confirm-zapping']}</h3><p>Pool:</p><p class="message-box-secondary">{csInfo['poolKey'] if pool == "custom" else pool}</p><p>{locale['send-tab-amount']}:</p><p class="message-box-secondary">{maxAmount:,.8f}</p>
                                                <button class="cancel-confirm-send-button" id="cancel-send-button" onclick="closeMessageBox()" type="button">{locale['cancel']}</button>
    ...
    inputDetails = TransactionInputs(
        txHistory, maxAmount, spendColdStake=spendCSOut, isZap=True, script=script
    )
...
async def sendTx():
    ...
    Element(
        "message-box"
    ).element.innerHTML = f"""<h3>{locale['please-confirm-sending']}</h3><p>{locale['menu-tab-item-address-label']}:</p><p class="message-box-secondary">{addr}</p><p>{locale['send-tab-amount']}:</p><p class="message-box-secondary">{amount:,.8f}</p>
                                                <button class="cancel-confirm-send-button" id="cancel-send-button" onclick="closeMessageBox()" type="button">{locale['cancel']}</button>
                                                <button class="cancel-confirm-send-button" id="confirm-send-button" onclick="finalizeSendTx()" type="button">{locale['send-tab-send-button']}</button>"""
    ...
    try:
        token = str(
            await password_decrypt(
                str(await getData("TOKEN").then(lambda x: x)), password
            )
        )
        return True
    except Exception as e:
...
async def genWallet():
    ...
    wallet = {
        ...
        "master_xpriv": str(
            await password_encrypt(
                str(mnemonic.xpriv).encode(), password_input.element.value
            )
        ),
        "xpub": str(mnemonic.derived_xpub),
        "xpriv": str(
            await password_encrypt(
                str(mnemonic.derived_xpriv).encode(), password_input.element.value
            )
        ),
        "xpub_change": str(mnemonic.derived_xpub_change),
        "xpriv_change": str(
            await password_encrypt(
                str(mnemonic.derived_xpriv_change).encode(),
                password_input.element.value,
            )
        ),
        "words": str(
            await password_encrypt(
                str(mnemonic.words).encode(), password_input.element.value
            )
        ),
        "utxo": [],
        "totalBalance": 0,
    ...
    await storeData(
        ...
        str(
            await password_encrypt(
                json.dumps(vars(importWallet)["wallet"]).encode(), token
            ),
            "utf-8",
        ),
    )
    await storeData(
        "TOKEN",
        str(
            await password_encrypt(token.encode(), password_input.element.value),
            "utf-8",
        ),
    )
...
async def expandSettings(setting):
    ...
    if isExpanded:
        js.jQuery(f"#{setting}").animate(
            to_js({"height": "40px"}, dict_converter=js.Object.fromEntries), fadeTime
        )
        await txHistory.setExpanded(setting, False)

    else:
        if (
            setting == "menu-tab-item-agvr"
            and txHistory.wallet.totalBalance < 2000000000000
        ):  # 20,000
            return

        height = js.jQuery(f"#{setting}").prop("scrollHeight") + 5
        js.jQuery(f"#{setting}").animate(
            to_js({"height": f"{height}px"}, dict_converter=js.Object.fromEntries),
            fadeTime,
        )
...
async def insertVets():
    ...
    addrs = (
        txHistory.wallet.receiving_addresses
        + txHistory.wallet.receiving_addresses_256
        + txHistory.wallet.change_addresses
    )
    ...
    if curVets:
        ...
        for vet in curVets:
            ...
            template = Element(f"current-vet-addr-template").select(
                ".addr-current-vet-item", from_content=True
            )
...
async def insertPools():
    ...
    for pool in txHistory.poolList:
        template = Element(f"coldstake-pool-template").select(
            ".menu-tab-coldstake-radio-label", from_content=True
        )
        ...
        coldstake_pool_name.element.innerHTML = (
            f'{poolName} <a href="{poolURL}" target="_blank">info</a>'
        )
    ...
    if not txHistory.wallet.coldstaking.get("spendKey"):
        txHistory.wallet.coldstaking[
            "spendKey"
        ] = txHistory.wallet.receiving_addresses_256[0]
        await txHistory.walletCls.flushWallet()

    Element("pool-spend-addr").element.innerText = txHistory.wallet.coldstaking[
        "spendKey"
    ]
...
async def setSpendAddr(addr):
    if (
        addr
        not in txHistory.wallet.receiving_addresses_256
        + txHistory.wallet.lookahead_addresses_256
    ):
        return
...
async def spendAddrSelection():
    ...
    for addr in availableAddr256:
        template = Element(f"new-cs-address").select(
            ".menu-tab-coldstake-radio-label", from_content=True
        )
    ...
    if selected == "disabled":
        ...
        if custom_pool_input.element.value != "":
            if getCsAddrInfo(custom_pool_input.element.value).isValid:
                ...
                txHistory.wallet.coldstaking[
                    "poolKey"
                ] = custom_pool_input.element.value
            else:
                custom_pool_input.element.style.backgroundColor = "#232728"
                custom_pool_input.element.style.color = "fafafa"
                menu_tab_coldstake_label.element.style.color = "#fafafa"
...
async def insertLang():
    ...
    for lang_item in lang_choice["langs"]:
        template = Element(f"lang-template").select(
            ".menu-tab-coldstake-radio-label", from_content=True
        )
...
async def insertFiat():
    ...
    for curr in currList:
        template = Element(f"fiat-template").select(
            ".menu-tab-coldstake-radio-label", from_content=True
        )
...
async def setAddrQrDisplay():
    ...
    if std_addr_index == None:
        ...
        addr_std = txHistory.wallet.receiving_addresses[
            txHistory.wallet.qr_standard_addr
        ]
    ...
    if addr_256_index == None:
        ...
        addr_256 = txHistory.wallet.receiving_addresses_256[
            txHistory.wallet.qr_256_addr
        ]
...
async def copyAddr(addrType):
    ...
    if addrType == "std":
        js.navigator.clipboard.writeText(
            str(txHistory.wallet.receiving_addresses[txHistory.wallet.qr_standard_addr])
        )
        qr_text_std.element.innerText = locale["qr_text_copy"]
        qr_text_std.element.style.color = "#aeff00"
        await asyncio.sleep(3)
        qr_text_std.element.innerText = str(
            txHistory.wallet.receiving_addresses[txHistory.wallet.qr_standard_addr]
        )
        qr_text_std.element.style.color = "#fafafa"

    elif addrType == 256:
        js.navigator.clipboard.writeText(
            str(txHistory.wallet.receiving_addresses_256[txHistory.wallet.qr_256_addr])
        )
        qr_text_256.element.innerText = locale["qr_text_copy"]
        qr_text_256.element.style.color = "#aeff00"
        await asyncio.sleep(3)
        qr_text_256.element.innerText = str(
            txHistory.wallet.receiving_addresses_256[txHistory.wallet.qr_256_addr]
        )
...
async def insertUsedAddresses():
    ...
    for addr in txHistory.wallet.used_addresses:
        ...
        template = Element(f"used-addr-template").select(
            ".addr-list-used-item", from_content=True
        )
...
async def newTx(txData):
    ...
    for i in list(txData["inputs"].keys()) + list(txData["outputs"].keys()):
        if (
            i
            in txHistory.wallet.receiving_addresses
            + txHistory.wallet.receiving_addresses_256
            + txHistory.wallet.change_addresses
        ):
            isMine = True
            ...
            i
            in txHistory.wallet.lookahead_addresses
            + txHistory.wallet.lookahead_addresses_256
            + txHistory.wallet.change_lookahead_addresses
        ):
            isMine = True
            isLookahead = True
...
async def updateBalanceDisplay():
    ...
    ).element.innerHTML = f'<span class="balance-display" style="">{int(bal[0]):,}</span><span>.{bal_small}</span>'
    ...
    spendable = round(
        convertFromSat(
            txHistory.wallet.totalBalance - txHistory.wallet.unconfirmedBalance - staked
        ),
        8,
    )
    ...
    if (
        txHistory.wallet.qr_standard_addr != None
        and txHistory.wallet.receiving_addresses[txHistory.wallet.qr_standard_addr]
        in txHistory.wallet.used_addresses
    ):
        Element("new-address-button-standard").element.disabled = False
    else:
        Element("new-address-button-standard").element.disabled = True
    ...
    if (
        txHistory.wallet.qr_256_addr != None
        and txHistory.wallet.receiving_addresses_256[txHistory.wallet.qr_256_addr]
        in txHistory.wallet.used_addresses
    ):
        Element("new-address-button-256").element.disabled = False
    else:
...
async def updateHistoryConfirms(pendingTx):
    for tx in txHistory.txHistory:
        ...
        if tx["txid"] == txHistory.showTxInfo:
            Element(f"msg-box-conf-{tx['txid']}").element.innerText = tx[
                "confirmations"
            ]
...
async def processNewTx(tx, isOldTx=False):
    ...
    template = Element(f"tx-history-template").select(
        ".tx-history-item", from_content=True
    )
    ...
    tx_html_date.element.innerHTML = (
        f"{'0' if txTime.day < 10 else ''}{txTime.day}•{'0' if txTime.month < 10 else ''}{txTime.month}•{str(txTime.year)[2:]} {'0' if txTime.hour < 10 else ''}{txTime.hour}:{'0' if txTime.minute < 10 else ''}{txTime.minute} "
        + f'<div class="confirm-progress" onclick="clickTXIDjs(`{tx["txid"]}`);" id="confirm-{tx["txid"]}" style="color: {confirm_dict[confirms]["color"]}">{confirm_dict[confirms]["text"]} </div>'
    )

    tx_html_amount.element.innerHTML = (
        f"{'+' if tx['txValue'] > 0 else ''}{tx['txValue']:,.8f} "
        + f'<div class="coin-name" id="coin-name">GHOST</div>'
    )
...
async def txInfo(txid):
    ...
    for addr in txIns:
        ...
        if addr["isMine"]:
            txDetailHTML += (
                f"""<p id="my-tx-in" style="color:#aeff00;">{addr['addr']}: {amt}</p>"""
            )
        else:
    ...
    offsetOut = (
        Element("my-tx-out").element.offsetTop
        - Element("tx-info-outputs").element.offsetTop
    )
    Element("tx-info-outputs").element.scrollTop = offsetOut - 42

    offsetIn = (
        Element("my-tx-in").element.offsetTop
        - Element("tx-info-inputs").element.offsetTop
    )
        ...
        newList.append(
            {
                "addr": i,
                "amount": addrDict[i],
                "isMine": await txHistory.walletCls.isMine(i),
            }
        )
...
async def insertShowMoreTx():
    ...
    template = Element(f"overview-tx-next-page").select(
        ".next-page-item", from_content=True
    )
...
async def getNextTxPage():
    ...
    await txHistory.processTxHistory(
        startIdx=best_tx, endIdx=best_tx + 50, advanceTopIndex=True
    )
...
async def getPrice():
    ...
    if fiat:
        ...
        total_price = round(
            price * txHistory.util.convertFromSat(txHistory.wallet.totalBalance), 6
        )
...
async def doTranslation(requested_locale=None):
    ...
    Element("balance-spendable-text").element.innerText = locale[
        "balance-spendable-text"
    ]
    Element("balance-coldstake-text").element.innerText = locale[
        "balance-coldstake-text"
    ]
    Element("balance-unconfirmed-text").element.innerText = locale[
        "balance-unconfirmed-text"
    ]
    ...
    Element("menu-tab-item-lang-label").element.innerText = locale[
        "menu-tab-item-lang-label"
    ]
    Element("menu-tab-item-address-label").element.innerText = locale[
        "menu-tab-item-address-label"
    ]
    Element("new-address-button-standard").element.innerText = locale[
        "new-address-button-standard"
    ]
    Element("new-address-button-256").element.innerText = locale[
        "new-address-button-256"
    ]
    Element("addr-list-used-text").element.innerText = locale["addr-list-used-text"]
    Element("menu-tab-item-coldstake-label").element.innerText = locale[
        "menu-tab-item-coldstake-label"
    ]
    Element("coldstake-pool-option-disabled-label").element.innerText = locale[
        "coldstake-pool-option-disabled"
    ]
    Element("coldstake-pool-option-custom-label").element.innerText = locale[
        "coldstake-pool-option-custom"
    ]
    Element("pool-spend-addr-text").element.innerText = locale["pool-spend-addr-text"]
    Element("current-vet-addr-label").element.innerText = locale[
        "current-vet-addr-label"
    ]
    Element("pending-vet-addr-label").element.innerText = locale[
        "pending-vet-addr-label"
    ]
    Element("menu-tab-item-fiat-label").element.innerText = locale[
        "menu-tab-item-fiat-label"
    ]
    Element("menu-tab-item-explorer-label").element.innerText = locale[
        "menu-tab-item-explorer-label"
    ]
    ...
    confirm_dict = {
        0: {"text": locale["unconfirmed"], "color": "#ff4d4d"},
        1: {"text": "1 / 12", "color": "#f85c47"},
        2: {"text": "2 / 12", "color": "#f16b40"},
        3: {"text": "3 / 12", "color": "#eb7a3a"},
        4: {"text": "4 / 12", "color": "#e48833"},
        5: {"text": "5 / 12", "color": "#dd972d"},
        6: {"text": "6 / 12", "color": "#d7a627"},
        7: {"text": "7 / 12", "color": "#d0b520"},
        8: {"text": "8 / 12", "color": "#c9c41a"},
        9: {"text": "9 / 12", "color": "#c2d313"},
        10: {"text": "10 / 12", "color": "#bbe10d"},
        11: {"text": "11 / 12", "color": "#b5f006"},
        12: {"text": locale["confirmed"], "color": "#aeff00"},
        "orphan": {"text": locale["orphaned"], "color": "#ff4d4d"},
    }
    ...
    if txHistory:
        for tx in txHistory.txHistory:
            ...
            if tx["confirmations"] >= 12:
                Element(f"confirm-{tx['txid']}").element.innerText = confirm_dict[12][
                    "text"
                ]
            elif tx["confirmations"] <= 0:
                Element(f"confirm-{tx['txid']}").element.innerText = confirm_dict[0][
                    "text"
                ]
